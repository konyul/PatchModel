...
class PatchnetSingleHead(patch_singlehead_BaseDecodeHead):
    ...
    def __init__(self,
        ...
        #         )
            ...
            conv_kernel_size = 3
            self.seg_head = nn.Sequential(
                nn.Conv2d(self.input_dim, self.input_dim//2, kernel_size=conv_kernel_size, padding=1),
                nn.SyncBatchNorm(self.input_dim//2),
                nn.ReLU(inplace=True),
                nn.Conv2d(self.input_dim//2, self.num_classes, kernel_size=1)
                )
            self.seg_head_dilated = nn.Sequential(
                nn.Conv2d(self.input_dim, self.input_dim//2, kernel_size=conv_kernel_size, padding=2, dilation=2),
                nn.SyncBatchNorm(self.input_dim//2),
                nn.ReLU(inplace=True),
                nn.Conv2d(self.input_dim//2, self.num_classes, kernel_size=1)
                )
            self.seg_head_dilatedv2 = nn.Sequential(
                nn.Conv2d(self.input_dim, self.input_dim//2, kernel_size=conv_kernel_size, padding=3, dilation=3),
                nn.SyncBatchNorm(self.input_dim//2),
                nn.ReLU(inplace=True),
                nn.Conv2d(self.input_dim//2, self.num_classes, kernel_size=1)
                )
